function countBalanced(strArr: string[]): number {
  let count = 0;
...
function countBalancedOpt(strArr: string[]): number {
  function countVC(str: string): [number, number] {
    let vowels = 0,
      consonants = 0;
    for (const ch of str) {
      if (/[a-z]/.test(ch)) {
  ...
  let count = 0;
  let balance = 0;
  const balancedMap = new Map<number, number>();
  balancedMap.set(0, 1);
...
const strArr: string[] = ["aeio", "aa", "bc", "ot", "cdbd"];
